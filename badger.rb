...
class Badger < Sinatra::Application
  ...
  post '/play' do
    ...
    if params["correctness"]
      ...
      guess_uuid, answer = @@librarian.record_guess({"quiz_uuid" => session[:quiz_uuid],
                                                     "guesser" => @player.name,
      ...
      puts "in play, answer: " + answer
      @options = Array.new
      if session[:options][0] == answer
        @options << answer
        @options << (@@names - [@player.name, answer]).sample
      else
        @options << (@@names - [@player.name, answer]).sample
        @options << answer
      end
    else # first time in the game
  ...
  post '/guess' do
    ...
    @quiz = @@librarian.get_a_quiz(session[:tester], params["answer"], session[:bundle_uuid])

    puts "cannot find any feasible quiz!" if @quiz == nil
      
    if (@quiz["option0"] == session[:options][0]) or 
       (@quiz["option1"] == session[:options][1])
      @to_switch_side = false
      session[:options] = [@quiz["option0"], @quiz["option1"]]
    else
      @to_switch_side = true
      session[:options] = [@quiz["option1"], @quiz["option0"]]
    end
